...
bool Enumerator::try_early_termination(const std::vector<int> &partial,
  ...
  // Enumerate and retain every terminal continuation. Runtime therefore
  // includes the per-clique traversal, construction, and storage cost.
...
// Identity-materializing output sink. Every HBBMC version reaches this path,
// so the numeric count and retained list cannot diverge. Validation mode also
// canonicalizes the clique and checks every invariant the README depends on:
...
void Enumerator::emit_clique(std::vector<int> clique, int owner_edge) {
  ...
  if (result_.maximal_clique_count ==
