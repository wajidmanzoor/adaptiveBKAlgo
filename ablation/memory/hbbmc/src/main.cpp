...
struct CliOptions {
  ...
  std::string graph_reduction = "rmce";
  int et_threshold = 3;
...
void usage(std::ostream &out, const char *program) {
  out << "Usage: " << program << " GRAPH [options]\n"
      ...
      << "  --graph-reduction NAME   none or rmce (default: rmce; alias: gr)\n"
...
// total identity count matches the numeric counters exactly.
...
int main(int argc, char **argv) {
  try {
    ...
    const std::uint64_t stored_clique_count =
        static_cast<std::uint64_t>(reduced.directly_emitted_cliques.size()) +
        static_cast<std::uint64_t>(result.cliques.size());
    if (stored_clique_count != maximal_clique_count) {
      throw std::logic_error(
          "stored HBBMC clique count differs from numeric count");
    }
    if (cli.validate) {
    ...
    std::cout << "graph_reduction_complete=" << (complete_gr ? "true" : "false")
              << '\n';
    std::cout << "early_termination_threshold=" << cli.et_threshold << '\n';
    ...
    std::cout << "stored_cliques=" << stored_clique_count << '\n';
    std::cout << "clique_storage=all\n";
    if (cli.validate) {
