...
terminal continuation, constructs `R` plus its clique suffix, and retains that
identity in the result list. It never replaces traversal with an algebraic
aggregate.
...
```sh
cmake -S main/hbbmc -B /tmp/hbbmc-faithful-build \
...
Every HBBMC, HBBMC+, HBBMC++, and diagnostic ET configuration retains every
reported clique identity. `--print-cliques` prints the already-retained list;
it does not change storage behavior. `stored_cliques` must equal
`maximal_cliques`, and `clique_storage=all` makes this contract machine
checkable. `--validate` additionally checks clique validity, global maximality,
uniqueness, and earliest-edge ownership. Reported `algorithm_runtime_ms`
includes graph reduction, truss ordering, enumeration, and clique-list
insertion, but excludes input parsing; the component times are also printed.

## Validation evidence

The implementation was developed against an exhaustive differential harness
before the source was trimmed into `finalCode`. That harness performed:
...
- retained identity versus numeric-count equality on every reduced pipeline;
...
The self-contained final package checks every retained-storage configuration
with:
...
Fresh Release and ASan/UBSan builds passed the development suite. HBBMC++
runs also reconcile the public totals on DBLP (125,568 direct plus
...
All configurations now include persistent clique-list storage so memory and
runtime comparisons with Pure account for the same output-retention policy.
Duplicate sets and owner checks remain validation-only. This remains a
