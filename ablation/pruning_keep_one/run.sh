...
timeout_seconds=${KEEP_ONE_TIMEOUT_SECONDS:-3600}
build_jobs=${KEEP_ONE_BUILD_JOBS:-4}
dataset_filter=${KEEP_ONE_DATASETS:-}
budget=${KEEP_ONE_BUDGET:-1000}

final_require_positive_integer KEEP_ONE_TIMEOUT_SECONDS "$timeout_seconds" || exit 2
final_require_positive_integer KEEP_ONE_BUILD_JOBS "$build_jobs" || exit 2
if [[ $budget != unlimited ]]; then
  final_require_nonnegative_integer KEEP_ONE_BUDGET "$budget" || exit 2
fi
...
for variant in "${variants[@]}"; do
  ...
  if [[ $variant == all_rules ]]; then
    ...
    enabled=${variant#only_}
    options=()
    for rule in "${FINAL_PRUNING_RULES[@]}"; do
      [[ $rule == "$enabled" ]] && continue
      options+=("-DPURE_DISABLE_PRUNING_${rule^^}=ON")
    done
    final_build "$variant" "$pure_source" "$build_root" \
      "$result_root/build_logs" "$build_jobs" "${options[@]}" || exit 2
  fi
...
{
  echo "campaign=Pure seven-rule keep-one pruning ablation"
  ...
  echo "variants=all_rules plus one variant per rule with the other six disabled"
  for variant in "${variants[@]}"; do
...
keep_one_config_matches() {
  local output=$1
  local enabled=$2
  local rule expected actual
  for rule in "${FINAL_PRUNING_RULES[@]}"; do
    expected=0
    if [[ $enabled == all || $rule == "$enabled" ]]; then
      expected=1
    fi
...
run_one() {
  ...
  if [[ $exit_code -eq 0 ]] &&
      ...
      keep_one_config_matches "$base.stdout" "$enabled"; then
    status=completed
  else
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$variant" "$enabled" "$graph" "$n" "$m" "$status" \
...
for group in "${FINAL_SELECTED_GROUPS[@]}"; do
  ...
  if [[ ! -f $runs_csv ]]; then
    printf '%s\n' \
      'variant,enabled_rule,graph,n,m,status,exit_code,cliques,wall_ms,runtime_ms,checks,budget_fallbacks,reference_count,count_match,slowdown_vs_all_rules' \
      >"$runs_csv"
  fi

  for variant in "${variants[@]}"; do
    enabled=all
    [[ $variant != all_rules ]] && enabled=${variant#only_}
    for spec in "${FINAL_SELECTED_DATASETS[@]}"; do
      IFS='|' read -r spec_group graph n m pure_input _ <<<"$spec"
      [[ $spec_group == "$group" ]] || continue
      run_one "$variant" "$enabled" "$graph" "$n" "$m" "$pure_input" \
        "$group_root" "$runs_csv"
    done
  done
...
if [[ ${KEEP_ONE_FAIL_ON_ERROR:-0} == 1 && $campaign_failed -ne 0 ]]; then
  exit 1
fi
