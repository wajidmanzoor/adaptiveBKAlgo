...
# Format: label order mode ord meth [hitSetLimit prune1 prune2 sp1 sp2 sp3 sp4 sp5 sp6]
# All ReorderSib runs use meth=1 (Optimized) with all prune flags = 0
RUNS=(
    "PivotBK   ORIG   0 0 0"
    "PivotBK   ASC    0 1 0"
    "PivotBK   DES    0 2 0"
    "Optimized ORIG   1 0 1 4294967295 0 0 0 0 0 0 0 0"
    "Optimized ASC    1 1 1 4294967295 0 0 0 0 0 0 0 0"
    "Optimized DES    1 2 1 4294967295 0 0 0 0 0 0 0 0"
)
...
for g in "${GRAPHS[@]}"; do
    ...
    for entry in "${RUNS[@]}"; do
        read -r label order mode ord meth extra <<< "$entry"

        # shellcheck disable=SC2086
        out=$(RUN "$BIN" "$g" "$mode" "$ord" "$meth" $extra 2>/dev/null) && rc=0 || rc=$?
