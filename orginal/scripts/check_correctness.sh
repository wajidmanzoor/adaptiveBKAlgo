...
if   command -v gtimeout &>/dev/null; then RUN() { gtimeout 300 "$@"; }
elif command -v  timeout &>/dev/null; then RUN() { timeout  300 "$@"; }
else                                        RUN() {               "$@"; }
fi

# ── Build ──────────────────────────────────────────────────────────────────────
cmake -S .. -B ../build -DCMAKE_BUILD_TYPE=Release > /dev/null 2>&1 \
  && cmake --build ../build -j$(nproc 2>/dev/null || sysctl -n hw.logicalcpu) > /dev/null 2>&1 \
