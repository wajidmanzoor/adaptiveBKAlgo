...
void DepthFirstReorderBK::recordClique(const vector<ui> &clique) {
  ...
  for (ui v : clique) cout << v << " ";
  cout << "}" << endl;
}

bool DepthFirstReorderBK::exploreFromClique(vector<ui> &current_clique, ui start_pos) {
  if (debug) {
  ...
  for (ui pos = start_pos; pos < global_order.size(); pos++) {
    ...
    if (canExtend(current_clique, v)) {
      ...
      if (debug) {
        ...
        for (ui x : extended_clique) cout << x << " ";
        cout << "}" << endl;
      }
      ...
      for (ui test_v : global_order) {
        if (find(extended_clique.begin(), extended_clique.end(), test_v) == extended_clique.end() &&
...
vector<ui> DepthFirstReorderBK::depthFirstExpandWithBacktrack(ui start_vertex) {
  ...
  if (debug) {
    cout << "Starting depth-first expansion with backtracking from vertex " << start_vertex << endl;
  }
...
void DepthFirstReorderBK::findAllMaximalCliques() {
  ...
  cout << "Starting Enhanced Depth-First Reordering Algorithm with Backtracking..." << endl;
  ...
  while (true) {
    ...
    if (debug) {
      cout << "\n=== Phase " << phase << ": Starting with vertex " << start_vertex << " ===" << endl;
    }
