...
size_t Edge_t::Hash_Edge_t::operator()(const Edge_t &e) const { return h(e); }
...
bool Graph_t::connect(int u, int v) { return cuhash[u].find(v); }
...
void Graph_t::er_model(int n, int rho) {
  ...
  FILE *fp =
      fopen((to_string((double)n / 1000000) + "x" + to_string(rho) + ".clean")
                .c_str(),
            "w");
  this->v_size = n;
  this->e_size = 0;
...
void HashMap_t::insert(const Edge_t &key, int val) {
  ...
  assert(table_size[pos] <
         MAX_COLL); // Enlarge max_coll or redefine hash function h(e)
...
void Heap_t::bubble_down(unsigned int i) {
  ...
  while (l < this->n) {
    j = ((r < this->n) && (this->kv_list[r].val < this->kv_list[l].val)) ? r
                                                                         : l;
...
bool Heap_t::empty() { return this->n == 0; }
...
bool Stack_t::empty() { return this->top == 0; }

void Stack_t::pop() { --this->top; }

void Stack_t::push(int v) { this->v_list[this->top++] = v; }

void Stack_t::print() {
  int i;
  for (i = 0; i < this->top; i++)
    printf("%d ", this->v_list[i]);
  printf("\n");
}
...
bool Queue_t::full() { return this->end == this->n; }
...
void Stat_t::collapse() {
  ...
  this->runtime =
      ((float)(t.ru_utime.tv_sec - s.ru_utime.tv_sec)) * 1e3 +
