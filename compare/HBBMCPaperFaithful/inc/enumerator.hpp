...
private:
  ...
  // Increments the running maximal-clique tally (count-only path); a no-op
  // if clique_size is below the reporting filter.
  void record_output_count(std::size_t clique_size, std::uint64_t count = 1U);
  // Identity-materializing output sink: always updates the numeric count via
  // record_output_count, and additionally validates/stores the clique
  // (deduplicated, sorted) when collect_cliques or validate_invariants is on.
