...
// validate the enumeration path in tests; the production Enumerator
// intentionally uses enumerate_t_plex_maximal_cliques even in count-only
// mode so its runtime includes one visit and construction per terminal
// clique. Throws on uint64_t overflow.
