...
#ifndef RETURN_CLIQUES_ONE_BY_ONE
#error "The local benchmark build must retain every reported clique."
#endif

static double elapsedMilliseconds(const struct timespec* start,
...
static void printStoredCliqueStats(const char* algorithm,
                                   long cliqueCount,
                                   LinkedList* cliques,
                                   const struct timespec* start,
                                   const struct timespec* end)
{
    long storedCliqueCount = (long)length(cliques);

    if(storedCliqueCount != cliqueCount)
    {
        fprintf(stderr,
                "stored clique count mismatch: enumerated=%ld stored=%ld\n",
                cliqueCount, storedCliqueCount);
        exit(EXIT_FAILURE);
    }

    printf("algorithm=%s\n", algorithm);
    printf("minimum_clique_size=%d\n", MINIMUM_CLIQUE_SIZE);
    printf("clique_storage=all\n");
    printf("clique_output=disabled\n");
    printf("maximal_cliques=%ld\n", cliqueCount);
    printf("stored_cliques=%ld\n", storedCliqueCount);
    printf("algorithm_wall_ms=%.3f\n", elapsedMilliseconds(start, end));
    fflush(stdout);
}
                            ...
                            int n )
{
    ...
    printStoredCliqueStats(algName, cliqueCount, cliques, &start, &end);
}
