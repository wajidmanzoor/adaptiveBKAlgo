...
namespace {
constexpr ui kMinimumCliqueSize = 3;
}

void MCE::saveCliques(const std::string & path) const {
    if(path.empty()) {
        throw std::invalid_argument("clique output path must not be empty");
    }

    std::ofstream output(path.c_str(), std::ios::out | std::ios::trunc);
    if(!output) {
        throw std::runtime_error("cannot open clique output file: " + path);
    }
    ...
    if(!output) {
        throw std::runtime_error("failed while writing clique output: " + path);
    }
}

void MCE::printStoredCliques(std::ostream & output) const {
    for(const auto & clique : allCliques) {
        output << "Clique:";
        for(ui vertex : clique) output << ' ' << vertex;
        output << '\n';
    }
    if(!output) {
        throw std::runtime_error("failed while printing stored cliques");
    }
}

void MCE::reportCurrentClique(ui EC, ui ER) {
    std::vector<ui> result;
    result.reserve(1 + EC + ER);

    auto originalId = [&](ui reorderedId) {
        const ui normalizedId = g.mp.empty() ? reorderedId : g.mp[reorderedId];
        return normalizedId + g.vertexIdOffset;
    };
    auto addLocalVertex = [&](ui localId) {
        const ui reorderedId = g.pEdge[g.pIdx[currentRoot] + localId];
        result.push_back(originalId(reorderedId));
    };

    result.push_back(originalId(currentRoot));
    for(ui i = 0; i < EC; ++i) addLocalVertex(Cv3[i]);
    for(ui i = 0; i < ER; ++i) addLocalVertex(Rv3[i]);
    std::sort(result.begin(), result.end());
    if(result.size() < kMinimumCliqueSize) {
        throw std::logic_error("CCRMCE attempted to report an undersized clique");
    }
    if(std::adjacent_find(result.begin(), result.end()) != result.end()) {
        throw std::logic_error("CCRMCE attempted to report a clique with duplicate vertices");
    }
    allCliques.push_back(std::move(result));
    cnt++;
}
...
printf("numOfDFSSearchNodes:%" PRIu64 "\n", numOfDFSSearchNodes);
#endif
#ifdef PERCENAEG_OF_C
printf("allCNodes:%" PRIu64 "\n", allCNodes);
printf("allPCNodes:%" PRIu64 "\n", allPCNodes);
printf("percentage:%f\n", 1.0*allCNodes/allPCNodes);
#endif
    if(static_cast<ull>(allCliques.size()) != static_cast<ull>(cnt)) {
        throw std::logic_error("stored clique count does not match CCRMCE count");
    }
    return cnt;
}
...
#endif
    // The current maximal clique consists of the implicit degeneracy root,
    // the vertices fixed in C, and the vertices selected into R.
    if(f && 1 + EC + ER >= kMinimumCliqueSize) {
#ifdef PERCENAEG_OF_C
allCNodes += EC;
allPCNodes += EC + ER;
#endif
        reportCurrentClique(EC, ER);
    }
